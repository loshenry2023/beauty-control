...
const deleteReg = async (dataInc) => {
    ...
    try {
        ...
        let regToDelete;
        let count = 0;
        if (tableNameText !== "Company" && tableNameText !== "Client" && tableNameText !== "Calendar" && tableNameText !== "Product") {
        ...
        switch (tableNameText) {
            case "Product":
                const { id: idProd, branchId } = id;
                // Verifico que no haya stock antes de permitir el borrado:
                const productBranch = await tableName2.findByPk(branchId); // Branch
                if (!productBranch) {
                    throw Error("Sede no encontrada");
                }
                const existingProd = await tableName.findOne({ // Product
                    where: { productCode: idProd, branchId: branchId },
                });
                if (!existingProd) {
                    throw Error("Insumo no encontrado");
                }
                if (existingProd.amount > 0) {
                    throw Error("No se puede eliminar porque tiene stock");
                }

                regDelete = existingProd.productName;
                await existingProd.destroy();
                logData({ op: "D", nameCompany: dataLog.nameCompany, dbName: dataLog.dbName, userName: dataLog.userName, desc: `Product ${regDelete} was deleted` });
                break;
            case "Client":
                // Elimino la relación con el cliente:
                regToDelete = await tableName.findByPk(id); // Client
                if (!regToDelete) { throw Error("Registro no encontrado"); }
                ...
                break;
            case "Calendar":
                regToDelete = await tableName.findByPk(id); //Calendar
                if (!regToDelete) { throw Error("Registro no encontrado"); }
